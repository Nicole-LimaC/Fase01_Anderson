...
class Pessoa(Connection):
    ...
    def insert(self, *args):
        try:
            sql = "INSERT INTO pessoa (titulo,sinopse,genero) VALUES (%s,%s,%s)"
    ...
    def insert_csv(self, filename):
        try:
            ...
            full_path = "c:/Users/Book/OneDrive/Documentos/Sistemas de Informação/Programação/Aula07_anderson/" + filename
            
            data = csv.DictReader(open(full_path, encoding="utf-8"))
            for row in data:
                self.insert(row["titulo","sinopse","genero"])                
